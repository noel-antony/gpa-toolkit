    ...
    >
      <div className="md:col-span-4">
        <input
          ...
          className="w-full rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 text-sm text-white transition focus:border-teal-400 focus:outline-none"
          placeholder="Subject name"
        />
      </div>

      <div className="flex items-center gap-2 md:col-span-1">
        <span className="md:hidden w-16 shrink-0 text-xs text-slate-400">Credits:</span>
        <input
          type="number"
          min="0"
          max="10"
          ...
          className={`w-full rounded-lg border bg-slate-950 px-3 py-2 text-sm ${
      ...
      <div className="flex items-center gap-2 md:col-span-1">
        <span className="md:hidden w-16 shrink-0 text-xs text-slate-400">Internal:</span>
        <input
          type="number"
          min="0"
          max="50"
      ...
      <div className="flex items-center gap-2 md:col-span-1">
        <span className="md:hidden w-16 shrink-0 text-xs text-slate-400">External:</span>
        <input
          type="number"
          min="0"
          max="100"
      ...
      <div className="flex items-center gap-2 md:col-span-1">
        <span className="md:hidden w-16 shrink-0 text-xs text-slate-400">Grade:</span>
        <select
          value={subject.grade}
          onChange={(event) => onUpdate(subject.id, { grade: event.target.value })}
          className={`w-full rounded-lg border bg-slate-950 px-3 py-2 text-sm transition focus:outline-none ${theme.text} ${theme.border}`}
        >
      ...
      <div className="flex items-center gap-2 md:col-span-1">
        <span className="md:hidden w-16 shrink-0 text-xs text-slate-400">Point:</span>
        <div className="w-full rounded-lg border border-slate-700 bg-slate-950/85 px-2 py-2 text-center text-sm font-bold">
          <p className={theme.text}>{metrics.point.toFixed(1)}</p>
        </div>
      </div>

      <div className="flex items-center gap-2 md:col-span-2">
        <span className="md:hidden w-16 shrink-0 text-xs text-slate-400">Result:</span>
        <div
          className={`w-full rounded-xl border px-3 py-2 text-center shadow-[0_0_14px_rgba(20,184,166,0.15)] md:col-span-2 ${theme.badge}`}
        >
      ...
      <div className="mt-2 flex justify-end md:col-span-1 md:mt-0 md:justify-center">
        <button
          type="button"
